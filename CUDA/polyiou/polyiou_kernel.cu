...
__device__ inline void polygon_cut(float2*p, int&n, float2 a, float2 b, float2* pp) {
	// TODO: The static variable may be the reason, why single thread is ok, 
	// multiple threads are not work
...
__device__ inline float devPolyIoU(float const * const dbbox1, 
	                               float const * const dbbox2) {
	...
	// printf("union_area: %f \n", union_area);
	float iou = 0.0f;
	if (sig(union_area) > 0) iou = inter_area / union_area;
	// printf("iou: %f \n", iou);
	return iou;
}
...
void _overlaps(float* overlaps, const float* boxes, const float* query_boxes, 
	...
	CUDA_CHECK(cudaMalloc(&boxes_dev, n * 5 * sizeof(float)));
	CUDA_CHECK(cudaMemcpy(boxes_dev, boxes, n * 5 * sizeof(float), 
		                  cudaMemcpyHostToDevice));

	CUDA_CHECK(cudaMalloc(&query_boxes_dev, k * 5 * sizeof(float)));
	CUDA_CHECK(cudaMemcpy(query_boxes_dev, query_boxes,
		                  k * 5 * sizeof(float), cudaMemcpyHostToDevice));

	CUDA_CHECK(cudaMalloc(&overlaps_dev, n * k * sizeof(float)));

	dim3 blocks(DIVUP(n, 32), DIVUP(k, 32));
	dim3 threads(32, 32);  // 每个线程块1024个线程
	overlaps_kernel<<<blocks, threads>>>(n, k, boxes_dev, query_boxes_dev,
		                                 overlaps_dev);

	CUDA_CHECK(cudaMemcpy(overlaps, overlaps_dev, n * k * sizeof(float),
